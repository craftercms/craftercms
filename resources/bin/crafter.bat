...
IF "%WITH_SOLR%"=="true" (
  ...
  call "%CRAFTER_BIN_DIR%\solr\bin\solr" start -p %SOLR_PORT% -s "%SOLR_HOME%" -Dcrafter.solr.index="%SOLR_INDEXES_DIR%"
)

IF NOT "%SKIP_ELASTICSEARCH%"=="true" (
  IF NOT EXIST "%ES_INDEXES_DIR%" mkdir "%ES_INDEXES_DIR%"
  start "ElasticSearch" cmd /c call "%ES_HOME%\elasticsearch" -d
)
