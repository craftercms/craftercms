...
start %CRAFTER_HOME%\solr\bin\solr start -f -p 8984 -Dcrafter.solr.index=%CRAFTER_HOME%\data\indexes
goto cleanOnExit
...
:debug
call %CRAFTER_HOME%\solr\bin\solr start -Dcrafter.solr.index=%CRAFTER_HOME%\data\indexes -f -p 8984 -a "-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=1044"
cd %DEPLOYER_HOME%
call %DEPLOYER_DEBUG%
cd %CRAFTER_HOME%
