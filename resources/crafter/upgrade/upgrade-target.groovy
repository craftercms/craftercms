...
def backupData(binFolder, dataFolder) {
	...
	if (Files.exists(dataFolder.resolve("repos"))) {
		executeCommand(binFolder, [SystemUtils.IS_OS_WINDOWS ? "crafter.bat" : "./crafter.sh", "backup"])
	} else {
 ...
 */
def backupBin(binFolder, backupsFolder) {
	println "============================================================"
...
def shutdownCrafter(binFolder) {
	...
	executeCommand(binFolder, [SystemUtils.IS_OS_WINDOWS ? "shutdown.bat" : "./shutdown.sh"], setupCallback)
}
...
def startCrafter(binFolder) {
	...
	executeCommand(binFolder, [SystemUtils.IS_OS_WINDOWS ? "startup.bat" : "./startup.sh"], setupCallback)
}

/**
 * Executes the upgrade.
 */
def upgrade(targetFolder, fullUpgrade) {
	...
	backupBin(binFolder, backupsFolder)
