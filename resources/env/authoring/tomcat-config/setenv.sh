...
CATALINA_OPTS="$CATALINA_OPTS -Dtomcat.host=$TOMCAT_HOST -Dtomcat.http.port=$TOMCAT_HTTP_PORT \
  ...
  -Dmariadb.port=$MARIADB_PORT -Dcrafter.solr.url=$SOLR_URL -Dcrafter.es.url=$ES_URL \
  -Dcrafter.deployer.url=$DEPLOYER_URL -Dcrafter.studio.url=$STUDIO_URL -Dcrafter.engine.url=$ENGINE_URL \
