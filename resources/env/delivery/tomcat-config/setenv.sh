...
CATALINA_OPTS="$CATALINA_OPTS -Dtomcat.host=$TOMCAT_HOST -Dtomcat.http.port=$TOMCAT_HTTP_PORT \
  ...
  -Dmail.port=$MAIL_PORT -Dmongodb.host=$MONGODB_HOST -Dmongodb.port=$MONGODB_PORT  -Dsolr.url=$SOLR_URL \
  -Des.url=$ES_URL -Dcrafter.home=$CRAFTER_HOME -Dcrafter.bin.dir=$CRAFTER_BIN_DIR \
  -Dcrafter.data.dir=$CRAFTER_DATA_DIR -Dcrafter.logs.dir=$CRAFTER_LOGS_DIR -Dcatalina.logs=$CATALINA_LOGS_DIR \
  -Dapplication.logs=$CATALINA_LOGS_DIR -Djava.net.preferIPv4Stack=true"
